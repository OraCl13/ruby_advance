...
feature 'Delete question and answer', %q{
  ...
  scenario 'Deleting ours questions&answers' do # save_and_open_page
    sign_in(user1)

    create_question
    create_answer

    expect(page).to have_content 'Destroy question'
    expect(page).to have_content 'Destroy answer'
  ...
  scenario 'Deleting not ours questions&answers' do
    sign_in(user1)
    create_question
    create_answer
    click_on 'Вихід'
